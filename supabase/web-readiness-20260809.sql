...
drop policy if exists "Feedback readable by all" on feedback;
...
drop policy if exists "Service role deletes feedback" on feedback;
create policy "Service role deletes feedback"
  on feedback for delete using (auth.role() = 'service_role');
